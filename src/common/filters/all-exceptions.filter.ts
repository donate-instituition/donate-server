...
import { sanitizeValue } from '../sanitization/sanitization';
...
function parseSource(stackLines: string[]) {
  const sourceLine = stackLines.find(
    (line) =>
      line.startsWith('at ') &&
      (line.includes('/src/') || line.includes('/dist/')),
  );
  ...
  const match = sourceLine.match(
    /^at\s+(?:(?<method>.*?)\s+\()?((?<file>.*?):(?<line>\d+):(?<column>\d+))\)?$/,
  );
  ...
  return {
    ...
    sourceColumn: match?.groups?.column
...
export class AllExceptionsFilter implements ExceptionFilter {
  ...
  constructor(private readonly errorLogsService: ErrorLogsService) {}
    ...
    void this.errorLogsService.create({
      ...
      ...source,
      userId:
        request.user?.sub && Types.ObjectId.isValid(request.user.sub)
          ? new Types.ObjectId(request.user.sub)
          : undefined,
      ip: request.ip,
      userAgent: request.headers['user-agent'],
      request: {
        params: sanitizeValue(request.params) as Record<string, unknown>,
        query: sanitizeValue(request.query) as Record<string, unknown>,
        body: sanitizeValue(request.body) as Record<string, unknown>,
      },
      stack,
      kafka: {
        queued: false,
